...
import { Bot, SendHorizontal, AlertTriangle, CheckCircle2, XCircle } from 'lucide-react';
...
const analyzeCaseStrength = (messages: Array<{ role: string; content: string }>): FlagAnalysis => {
  const userMessages = messages.filter(m => m.role === 'user');
  const combinedContent = userMessages.map(m => m.content.toLowerCase()).join(' ');
  
  if (combinedContent.includes('death') || combinedContent.includes('permanent') || combinedContent.includes('severe')) {
    return {
      flag: 'red',
      title: 'Strong Case Indicated',
      description: 'Based on the information provided, your case shows strong indicators of medical malpractice. We recommend immediate legal consultation.'
    };
  } else if (combinedContent.includes('pain') || combinedContent.includes('error') || combinedContent.includes('mistake')) {
    return {
      flag: 'yellow',
      title: 'Potential Case - Further Review Needed',
      description: 'Your case shows potential merit but requires additional review. We recommend providing more details or consulting with a lawyer.'
    };
  } else {
    return {
      flag: 'green',
      title: 'Limited Case Indicators',
      description: 'Based on current information, the case may face challenges. Consider providing more details or consulting with a lawyer for a thorough review.'
    };
  }
};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (userMessageCount === 5) {
        const result = analyzeCaseStrength([...messages, { role: 'user', content: userMessage }]);
        setAnalysis(result);
      }
          ...
          <CardContent className="flex-1 flex flex-col p-4 min-h-0">
            <ScrollArea className="flex-1 pr-4 min-h-0">
              <div className="space-y-4">
                ...
                {analysis && (
                  <Alert className={cn("mt-4 border-2", getFlagColor(analysis.flag))}>
